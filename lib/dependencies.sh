install_oracle_libraries(){
  ...
  curl -k "$basic_download_url" --silent --fail --retry 5 --retry-max-time 15 -o instantclient-basic-linux.x64-12.1.0.2.0
  echo "Downloaded [$basic_download_url]"
  curl -k "$sdk_download_url" --silent --fail --retry 5 --retry-max-time 15 -o instantclient-sdk-linux.x64-12.1.0.2.0
